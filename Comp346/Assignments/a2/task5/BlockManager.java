package a2.task5;// Import (aka include) some stuff.
import a2.task5.common.BaseThread;
import a2.task5.common.Semaphore;
...
public class BlockManager
{
	 ...
	 */
	private static Semaphore s1 = new Semaphore(-9);				//task4
	...
	private  static boolean firstTimeUnlock = true;
		...
		public void run()
		{
			...
			if (firstTimeUnlock){
				for (int i = 0; i < siThreadSteps; i++) {
					System.out.println("All the threads finished PHASE I");
				}
				firstTimeUnlock = false;
			}
			mutex.V();
			s1.V();
			...
			while (!turnTestAndSet()){
				System.out.println("Thread [" + this.iTID + "] is trying to start phase II, but fails because of incorrect order.");
				s2.V();
				try {
					sleep(10);
				} catch (InterruptedException e) {
			...
			if (this.iTID == 10){
				System.out.println("\nAll the threads finished PHASE II");
			}
		}
	} // class AcquireBlock
